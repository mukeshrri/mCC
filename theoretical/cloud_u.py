# Plot column density and EW for uniform distribution 
...
EW = np.loadtxt("./data/ew_u_{}_{}_{}.txt".format(int(np.log10(Mcold)),int(np.log10(Ncc)),Rcc))
...
#np.savetxt('mean_EW_u.txt', np.column_stack(((rad_bin[:-1]+rad_bin[1:])/2.,EW_avg)))  

No_cc = 2.*Ncc/(4./3.*np.pi*rCGM**3.)*np.pi*Rcc**2.*r_perp*np.sqrt((rCGM/r_perp)**2. - 1.)
...
plt.text(50,1.2e14,'uniform distribution', fontsize=20., color='black')  
axs.set_yscale('log')
axs.set_xscale('log')
axs.set_xlim(10,300)
...
plt.text(80,3.5,r'uniform distribution', fontsize=22, color='black')
axs.set_yscale('log')
axs.set_xscale('log')
axs.set_xlim(10,300)
