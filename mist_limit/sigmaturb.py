...
for ii in range(Rcc.shape[0]):
   fl = open("./data/dataturb_{}_{}_{}_{}.pickle".format(n_cold,Rcc[ii],rc,np.log10(Mcc)),'rb')
   ...
   plt.hist(s_turb, bins=np.arange(0,max(s_turb),1), align='mid', color=color[ii], density=True, stacked=True, histtype='step', lw=4, ls='-', label=r'$R_{{\rm cc}}={}$'.format(Rcc[ii]))
   
   x = np.arange(1,9,1)
   mean = np.mean(s_turb)
   pois = np.zeros((x.shape[0]))
   for j in range(x.shape[0]):
     pois[j] = mean**x[j] * np.exp(-mean) / math.factorial(x[j])
     
   axs.plot(x,pois, color=color[ii], ls='--', lw=4)  
...
fig.tight_layout()
plt.savefig('./figures/sigma_turb_hist.png')
plt.show()
plt.close()   
...
for i in range(Rcc.shape[0]):
   axs.scatter(fa_T[i], meanturb[i]/sigturb[i], s=300, color=color[i], edgecolors='black', label=r'$R_{{\rm cc}}={}$'.format(Rcc[i]))
   #axs.scatter(Rcc[i], meanturb[i]/sigturb[i], s=300, color=color[i], edgecolors='black', label=r'$R_{{\rm cc}}={}$'.format(Rcc[i]))
...
fig.tight_layout()
#plt.savefig('./figures/fac_sigratio.png')
plt.savefig('./figures/fac_sigratio.pdf')
plt.show()
plt.close()
...
axs.scatter(Rcc, meanturb, color='magenta', s=200, label=r'mean$(\sigma_{\rm turb})$')
#axs.scatter(Rcc[-1], meanturb[-1], color='magenta', edgecolors='black', s=200)

axs.scatter(Rcc, sigturb, color='deepskyblue', s=200, label=r'std$(\sigma_{\rm turb})$')
#axs.scatter(Rcc[-1], sigturb[-1], color='skyblue', edgecolors='black', s=200)

axs.set_title(r'r$_{{\rm cl}}$ = {} pc'.format(rc), fontsize=24)
...
axs.set_xlabel(r'$R_{\rm cc}$ (kpc)', fontsize=24)
axs.legend(loc='upper right', fontsize=16)
fig.tight_layout()
#plt.savefig('./figures/sigmaturb.png')
plt.show()
plt.close()   
